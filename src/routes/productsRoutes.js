...
import { productExample, productExample2, productExample3 } from "../ProductManager/products.js";
...
productsRoutes.put("/:id", async (req, res) => {
  ...
  try {
    const productToUpdate = await manager.updateProduct(+id, update);
...
productsRoutes.delete("/:id", async (req, res) => {
  const id = req.params.id;
  try {
    const deletedProduct = await manager.deleteProduct(parseInt(id));
